...
-- ============================================
--    AUTH — password required ONCE per 48h
--    saved via writefile/readfile
-- ============================================
local AUTH_FILE  = "arceus_auth.txt"
local EXPIRE_H   = 48  -- hours

local function getTime() return os.time() end

local function isAuthed()
    local ok, data = pcall(readfile, AUTH_FILE)
    if not ok or not data or data=="" then return false end
    local saved = tonumber(data)
    if not saved then return false end
    return (getTime() - saved) < (EXPIRE_H * 3600)
end

local function saveAuth()
    pcall(writefile, AUTH_FILE, tostring(getTime()))
end

-- if already authed within 48h, skip straight to hub
if isAuthed() then
    task.spawn(_G.__ArceusHubLaunch or function() end)
    -- will be defined below; we patch this after definition
else

local AuthGui = Instance.new("ScreenGui")
AuthGui.Name="ArceusAuth"; AuthGui.ResetOnSpawn=false
AuthGui.IgnoreGuiInset=true; AuthGui.ZIndexBehavior=Enum.ZIndexBehavior.Sibling
AuthGui.Parent=player.PlayerGui

local Overlay=Instance.new("Frame",AuthGui)
Overlay.Size=UDim2.new(1,0,1,0); Overlay.BackgroundColor3=Color3.fromRGB(5,4,14)
Overlay.BackgroundTransparency=0.3; Overlay.BorderSizePixel=0

local AuthBox=Instance.new("Frame",AuthGui)
AuthBox.Size=UDim2.new(0,340,0,240); AuthBox.Position=UDim2.new(0.5,-170,0.5,-120)
AuthBox.BackgroundColor3=Color3.fromRGB(12,12,18); AuthBox.BorderSizePixel=0
Instance.new("UICorner",AuthBox).CornerRadius=UDim.new(0,16)
local ABS=Instance.new("UIStroke",AuthBox); ABS.Thickness=2

task.spawn(function()
    local t=0
    while AuthBox.Parent do
        t=t+task.wait(0.05)
        local r=0.5+0.5*math.sin(t)
        ABS.Color=Color3.fromRGB(math.floor(180+75*r),math.floor(180+75*r),math.floor(180+75*r))
    end
end)

local AuthBar=Instance.new("Frame",AuthBox)
AuthBar.Size=UDim2.new(1,0,0,3); AuthBar.BackgroundColor3=Color3.fromRGB(255,255,255); AuthBar.BorderSizePixel=0
local AuthBarG=Instance.new("UIGradient",AuthBar)
AuthBarG.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,Color3.fromRGB(100,100,100)),ColorSequenceKeypoint.new(0.5,Color3.fromRGB(255,255,255)),ColorSequenceKeypoint.new(1,Color3.fromRGB(100,100,100))})

local AuthTitle=Instance.new("TextLabel",AuthBox)
AuthTitle.Size=UDim2.new(1,0,0,46); AuthTitle.Position=UDim2.new(0,0,0,14)
AuthTitle.BackgroundTransparency=1; AuthTitle.Text="⚡ ARCEUS HUB"
AuthTitle.TextColor3=Color3.fromRGB(255,255,255); AuthTitle.TextSize=24; AuthTitle.Font=Enum.Font.GothamBold

local AuthSub=Instance.new("TextLabel",AuthBox)
AuthSub.Size=UDim2.new(1,0,0,18); AuthSub.Position=UDim2.new(0,0,0,58)
AuthSub.BackgroundTransparency=1; AuthSub.Text="Enter password  —  valid for 48h"
AuthSub.TextColor3=Color3.fromRGB(180,180,180); AuthSub.TextSize=12; AuthSub.Font=Enum.Font.Gotham

local AuthInput=Instance.new("TextBox",AuthBox)
AuthInput.Size=UDim2.new(1,-40,0,38); AuthInput.Position=UDim2.new(0,20,0,90)
AuthInput.BackgroundColor3=Color3.fromRGB(20,20,28); AuthInput.BorderSizePixel=0
AuthInput.Text=""; AuthInput.PlaceholderText="Password..."
AuthInput.TextColor3=Color3.fromRGB(230,230,230); AuthInput.PlaceholderColor3=Color3.fromRGB(100,100,120)
AuthInput.TextSize=14; AuthInput.Font=Enum.Font.Gotham; AuthInput.ClearTextOnFocus=false
Instance.new("UICorner",AuthInput).CornerRadius=UDim.new(0,10)
local AIS=Instance.new("UIStroke",AuthInput); AIS.Color=Color3.fromRGB(70,70,85); AIS.Thickness=1
local AIP=Instance.new("UIPadding",AuthInput); AIP.PaddingLeft=UDim.new(0,12)

local AuthStatus=Instance.new("TextLabel",AuthBox)
AuthStatus.Size=UDim2.new(1,0,0,18); AuthStatus.Position=UDim2.new(0,0,0,136)
AuthStatus.BackgroundTransparency=1; AuthStatus.Text=""; AuthStatus.TextSize=12; AuthStatus.Font=Enum.Font.GothamBold

local AuthBtn=Instance.new("TextButton",AuthBox)
AuthBtn.Size=UDim2.new(1,-40,0,40); AuthBtn.Position=UDim2.new(0,20,0,162)
AuthBtn.BackgroundColor3=Color3.fromRGB(55,55,70); AuthBtn.Text="UNLOCK"
AuthBtn.TextColor3=Color3.fromRGB(240,240,245); AuthBtn.TextSize=15
AuthBtn.Font=Enum.Font.GothamBold; AuthBtn.BorderSizePixel=0
Instance.new("UICorner",AuthBtn).CornerRadius=UDim.new(0,10)
AuthBtn.MouseEnter:Connect(function() TweenService:Create(AuthBtn,TweenInfo.new(0.12),{BackgroundColor3=Color3.fromRGB(90,90,110)}):Play() end)
AuthBtn.MouseLeave:Connect(function() TweenService:Create(AuthBtn,TweenInfo.new(0.12),{BackgroundColor3=Color3.fromRGB(55,55,70)}):Play() end)

-- small "remember 48h" note
local AuthNote=Instance.new("TextLabel",AuthBox)
AuthNote.Size=UDim2.new(1,0,0,16); AuthNote.Position=UDim2.new(0,0,0,208)
AuthNote.BackgroundTransparency=1; AuthNote.Text="⏱  After unlocking you won't need to type it again for 48h"
AuthNote.TextColor3=Color3.fromRGB(120,120,120); AuthNote.TextSize=10; AuthNote.Font=Enum.Font.Gotham

local function shakeBox()
    for i=1,6 do
        TweenService:Create(AuthBox,TweenInfo.new(0.04),{Position=UDim2.new(0.5,-170+(i%2==0 and 8 or -8),0.5,-120)}):Play()
        task.wait(0.04)
    end
    TweenService:Create(AuthBox,TweenInfo.new(0.08),{Position=UDim2.new(0.5,-170,0.5,-120)}):Play()
end
...
local function tryAuth()
    if AuthInput.Text==PASSWORD then
        saveAuth()
        AuthStatus.Text="✅ Access granted! Saved for 48h"
        AuthStatus.TextColor3=Color3.fromRGB(100,255,160)
        TweenService:Create(AuthBox,TweenInfo.new(0.3,Enum.EasingStyle.Back,Enum.EasingDirection.In),{Size=UDim2.new(0,340,0,0),Position=UDim2.new(0.5,-170,0.5,0)}):Play()
        task.delay(0.35,function()
            AuthGui:Destroy()
            _G.__ArceusHubLaunch()
        end)
    else
        attempts=attempts+1
        AuthStatus.Text="❌ Wrong password! ("..attempts.." attempt"..(attempts>1 and "s" or "")..")"
        AuthStatus.TextColor3=Color3.fromRGB(255,80,80)
        AuthInput.Text=""
        task.spawn(shakeBox)
        TweenService:Create(AIS,TweenInfo.new(0.1),{Color=Color3.fromRGB(255,60,60)}):Play()
        task.delay(1,function() TweenService:Create(AIS,TweenInfo.new(0.3),{Color=Color3.fromRGB(70,70,85)}):Play() end)
    end
end

AuthBtn.MouseButton1Click:Connect(tryAuth)
AuthInput.FocusLost:Connect(function(e) if e then tryAuth() end end)

end -- end else (not authed)

-- ============================================
--         MAIN HUB (launched after auth)
-- ============================================
function _G.__ArceusHubLaunch()
...
local KEYBIND_FILE = "arceus_keybinds.txt"
...
local function saveKeybinds()
    ...
    pcall(writefile, KEYBIND_FILE, table.concat(out, "
"))
end
...
")
    for i,line in ipairs(lines) do
        if keybinds[i] and line~="" then
            local parts=string.split(line,"|")
            local lbl=parts[1]; local key=parts[2]
            -- find matching action
            for _,act in ipairs(ACTIONS) do
                if act.label==lbl then
                    keybinds[i].label=lbl
                    keybinds[i].fn=act.fn
                    keybinds[i].key=key~="" and key or nil
                    if keybinds[i].actionBtn then keybinds[i].actionBtn.Text=lbl end
                    if keybinds[i].keyBtn then keybinds[i].keyBtn.Text=key~="" and "[ "..key.." ]" or "[ none ]" end
                    break
                end
            end
        end
